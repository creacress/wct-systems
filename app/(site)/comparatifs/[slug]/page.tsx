...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: c.keywords,
    alternates: { canonical: `/comparatifs/${c.slug}` },
  };
}
    ...
    "@graph": [
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: c.title, item: `${SITE_URL}/comparatifs/${c.slug}` },
        ],
      },
      {
        "@type": "FAQPage",
      ...
      <div className="mx-auto max-w-5xl px-6 py-16 sm:py-20">
        ...
        <ScrollReveal direction="up">
          <section className="space-y-6">
            ...
            <p className="max-w-3xl text-lg text-muted-foreground">{c.intro}</p>
          </section>
        </ScrollReveal>

        {/* TABLEAU COMPARATIF — Desktop */}
        <ScrollReveal direction="up" delay={60}>
          <section className="mt-14" aria-label="Tableau comparatif">
            <h2 className="font-display font-bold text-xl tracking-tight mb-6">
              {c.optionAName} vs {c.optionBName} vs WCT Systems
            </h2>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr>
                    <th className="text-left py-3 px-4 font-medium text-muted-foreground border-b dark:border-white/[0.08]">Critère</th>
                    <th className="text-left py-3 px-4 font-medium border-b dark:border-white/[0.08]">{c.optionAName}</th>
                    <th className="text-left py-3 px-4 font-medium border-b dark:border-white/[0.08]">{c.optionBName}</th>
                ...
                <div key={i} className="rounded-2xl border bg-background/60 p-4 shadow-sm backdrop-blur dark:border-white/[0.08] dark:bg-white/[0.03]">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between gap-2">
                      <span className="text-muted-foreground shrink-0">{c.optionAName}</span>
                      <span className="text-right">{f.optionA}</span>
                    </div>
                    <div className="flex justify-between gap-2">
                      <span className="text-muted-foreground shrink-0">{c.optionBName}</span>
          ...
          <section className="mt-14">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <div className="rounded-3xl border bg-background/60 p-6 shadow-sm backdrop-blur dark:border-white/[0.08] dark:bg-white/[0.03]">
                <h3 className="font-bold tracking-tight mb-4">{c.optionAName}</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-xs font-medium text-emerald-600 dark:text-emerald-400 uppercase tracking-wide mb-2">Avantages</p>
                  ...
                  <div>
                    <p className="text-xs font-medium text-red-600 dark:text-red-400 uppercase tracking-wide mb-2">Inconvénients</p>
                    <ul className="space-y-1.5">
                      {c.consA.map((p) => (
                        <li key={p} className="flex items-start gap-2 text-sm text-muted-foreground">
              ...
              {/* Option B */}
              <div className="rounded-3xl border bg-background/60 p-6 shadow-sm backdrop-blur dark:border-white/[0.08] dark:bg-white/[0.03]">
                <h3 className="font-bold tracking-tight mb-4">{c.optionBName}</h3>
                <div className="space-y-4">
                  <div>
                    <p className="text-xs font-medium text-emerald-600 dark:text-emerald-400 uppercase tracking-wide mb-2">Avantages</p>
                    <ul className="space-y-1.5">
          ...
          <section className="mt-14">
            <div className="rounded-3xl border border-violet-200/60 bg-linear-to-r from-violet-50/50 to-indigo-50/30 p-8 dark:border-violet-900/20 dark:from-violet-950/20 dark:to-indigo-950/10">
              ...
              <div className="mt-6">
                ...
                >
                  Voir le service {c.serviceLabel} &rarr;
                </TrackedLink>
              </div>
            </div>
          </section>
        ...
        <ScrollReveal direction="up" delay={120}>
          <section className="mt-14">
            <h2 className="font-display font-bold text-xl tracking-tight mb-4">Notre verdict</h2>
            <div className="rounded-3xl border bg-background/60 p-6 shadow-sm backdrop-blur dark:border-white/[0.08] dark:bg-white/[0.03]">
              <p className="text-muted-foreground leading-relaxed">{c.verdict}</p>
            </div>
          </section>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up" delay={140}>
          <section className="mt-14">
            <h2 className="font-display font-bold text-xl tracking-tight mb-6">Questions fréquentes</h2>
        ...
        <ScrollReveal direction="up" delay={160}>
          <section className="mt-16 rounded-3xl border border-violet-100/70 bg-linear-to-r from-violet-50/50 to-indigo-50/30 p-10 text-center dark:border-violet-900/20 dark:from-violet-950/25 dark:to-indigo-950/15">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              Audit gratuit de 15 min. On analyse votre situation et on vous recommande la meilleure approche.
            </p>
