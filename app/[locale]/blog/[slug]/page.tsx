...
export async function generateMetadata({
  ...
  return {
    title: `${post.title} | ${t("post.metaTitleSuffix")}`,
    description: post.description,
    keywords: post.tags,
    alternates: {
      canonical: `${SITE_URL}${locale === "pt" ? `/pt/blog/${slug}` : `/blog/${slug}`}`,
      languages: {
        fr: `${SITE_URL}/blog/${slug}`,
        "pt-PT": `${SITE_URL}/pt/blog/${slug}`,
...
export default async function BlogPostPage({
  ...
  const formattedDate = new Date(post.date).toLocaleDateString(locale === "pt" ? "pt-PT" : "fr-FR", {
    day: "numeric",
    month: "long",
    year: "numeric",
  });
    ...
    "@graph": [
      {
        "@type": "BreadcrumbList",
        itemListElement: [
          { "@type": "ListItem", position: 1, name: t("post.breadcrumbHome"), item: `${SITE_URL}/` },
          { "@type": "ListItem", position: 2, name: t("post.breadcrumbBlog"), item: `${SITE_URL}/blog` },
  ...
  return (
    <main id="content" className="relative overflow-hidden">
      ...
      <article className="mx-auto max-w-3xl px-6 py-16 sm:py-20">
        {/* Breadcrumbs */}
        <nav aria-label={t("post.breadcrumbLabel")} className="mb-8 text-sm text-muted-foreground">
          <ol className="flex flex-wrap items-center gap-1.5">
            <li><Link href="/" className="hover:text-foreground transition-colors">{t("post.breadcrumbHome")}</Link></li>
            <li aria-hidden>/</li>
            <li><Link href="/blog" className="hover:text-foreground transition-colors">{t("post.breadcrumbBlog")}</Link></li>
            <li aria-hidden>/</li>
